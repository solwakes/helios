...
- ~~**`gfollow <src> <label>`** — thin wrapper over `SYS_FOLLOW_EDGE`. Completes the first-pass utility set.~~ **Shipped 2026-05-06** as `crates/gfollow-user/` (~140 LOC including doc, output ~6.5 KiB). `spawn gfollow <src> <label>` returns the target of the first outgoing edge from `<src>` whose label matches `<label>`. Cap-grant: `traverse` on `<src>` + `read` on a long-lived `gfollow-label-buf` Text node into which the shell stuffs the label bytes pre-spawn. Reusing one shared buf node across spawns is safe because `cmd_spawn` is synchronous (`run_user_task_with_caps` blocks until task exit). Tested in QEMU: happy path returns `-> #2` for `spawn gfollow 1 child`; NotFound prints `no edge "<label>" from <src>` and exits 2; bad source prints `EPERM (no traverse cap to #X)` and exits 1; consecutive invocations confirm the buf is correctly rewritten each spawn. UART transcript at `screenshots/gfollow-uart.txt`.
- ~~**`gwrite <id> <content>`** — exercises `SYS_WRITE_NODE`. Demo program for write caps.~~ **Shipped 2026-05-07** as `crates/gwrite-user/` (~120 LOC including doc, output ~5.7 KiB). `spawn gwrite <id> <content>` overwrites `<id>`'s content with the given byte string. Cap-grant: `write` on `<id>` + `read` on a long-lived `gwrite-content-buf` Text node into which the shell stuffs the content bytes pre-spawn. Same single-shared-buf pattern as gfollow — safe because `cmd_spawn` is synchronous. Multi-word content works because `execute()` does `splitn(4, ' ')`, so the third captured token is the entire remainder including spaces. Tested in QEMU on the M30 user-scratch node (#21): happy path writes `hello world` (11 bytes), `cat` confirms; rewrite with `second message` (14 bytes) confirms the buf is correctly rewritten across spawns; `spawn gwrite 99999 nope` falls through to `EPERM (no write cap to #99999)` because the shell-side `add_edge(write, 99999)` fails when the target doesn't exist (the user binary's `Errno::NotFound` arm is defensive — would only fire on a write-cap-followed-by-delete race); `spawn gwrite 0 broken` is rejected pre-spawn. Completes the first-pass utility set: list / read / recurse / step / write. UART transcript at `screenshots/gwrite-uart.txt`.
