...
//!
//! Mirrors `crates/mmap-user/build.rs`. See that file (and the
//! `hello-user/build.rs` it in turn mirrors) for rationale on why we
//! can't put this in `.cargo/config.toml`.
