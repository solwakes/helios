...
## Milestone Status (M1–M35 + M33.5 Complete)
...
- **M35 (done):** Cap delegation + CDT for revocation. Three phases on 2026-05-10/11/12. Phase 1 (commit `834d23c`): graph-layer tombstones + `derived_from` lineage + `EdgeId(src_node_id, vec_position)` + `Node::iter_live`. Phase 2 (commit `5591534`): `SYS_DELEGATE_EDGE` (syscall 10) + `SYS_REVOKE_EDGE` (syscall 11) + fifth cap label `grant` (`EDGE_KIND_GRANT = 5`, per-target, MMU-inert) + active-task cap-cache rebuild + per-target PT unmap + `sfence.vma` + task-exit cascade. Phase 3 (commit `c808003`): litmus binaries `cdtsmoke-alpha-user` (delegate-then-revoke within one lifetime, prints PASS/FAIL) and `cdtsmoke-beta-user` (delegate-then-exit, cascade-on-exit observable from α's baseline read of B). helios-std exposes `graph::delegate_edge(target, to_task, label)` and `graph::revoke_edge(target, label)`. Authority is now first-class user-space-mutable. See `docs/design/capability-edges.md` "M35 Implementation Notes" for the tombstones-vs-swap-remove / per-target-grant / active-task-only-PT-invalidation / paired-test-shape rationale. Proposal C from `docs/design/proposals/post-m32-directions.md` is fully closed.
- **M36+ planned:** Multiple coexisting user tasks (lifts the single-hart-cooperative simplifications M35 leaned on), port DOOM to user mode as the litmus test.
